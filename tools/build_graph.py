...
The register (docs/T9Sim_DGP_Node_Register.md) is the design input and stays the
source. This turns its 78 rows into the settings file the loader reads, so the
...
REG = ROOT / "docs" / "T9Sim_DGP_Node_Register.md"
SPEC = ROOT / "docs" / "T9Sim_Specification_v2.md"
...
def frozen_columns():
    """The 55 in deposit order, from specification 1.7. Startup check V4 compares
    the emitting rows against this, and the writer emits in this order so a
    re-run is byte-identical."""
    text = SPEC.read_text(encoding="utf-8")
    raw = re.search(r"55 columns\):\s*(.+?)\.\n", text, re.S).group(1)
    raw = raw.replace("\n", " ")
    raw = re.sub(r"(\w+)_\{([^}]+)\}",
...
def main():
    ...
    lines = [
        "# graph.yaml, the 78 generator variables.",
        "# GENERATED by tools/build_graph.py from docs/T9Sim_DGP_Node_Register.md.",
        ...
        "",
        "# The frozen 55, in deposit order, read from specification 1.7.",
        "column_order:",
