...
# THE v2.2 FORK. Left pointing at v2 this reported "mismatch in nodes,
# emitting rows, law T4" for step 5, which reads as a graph defect and was
# a stale path: v2's register has 78 rows and no H5. The same repointing was
# needed in tools/build_graph.py and tests/test_settings.py, 22 Aug 2026.
REGISTER = ROOT / "docs" / "T9Sim_DGP_Node_Register_v2.2.md"
...
CHECK_WHAT = {
    ...
    "V4": "the emitting rows are exactly the frozen column set",
...
def write_provenance(s):
    ...
    L = ["# v2 provenance register", "",
         ...
         "on every calibratable YAML leaf, meaning a terminal setting in the config tree. NB YAML leaf, always qualified: a bare \"leaf\" means a childless node in the DAG, of which there are 16. The two counts are unrelated. See docs/KDD_Terminology.md. This file is a readable view of that, nothing more. "
    ...
    L += ["| **total YAML leaves** | **%d** |" % len(rows), ""]
...
def main():
    ...
    print("      %d YAML leaves %s" % (sum(counts.values()), dict(counts)))
